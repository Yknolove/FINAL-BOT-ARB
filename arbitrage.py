...
async def fetch_binance_p2p(session: aiohttp.ClientSession, trade_type: str):
    payload = {"asset": "USDT", "fiat": "UAH", "tradeType": trade_type, "page": 1, "rows": 1}
    try:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if buy_price > 0 and sell_price > 0:
            ...
            try:
                ...
                logging.error(f"DB error on table creation: {oe}")
            cur = conn.cursor()
            try:
                cur.execute("SELECT user_id FROM settings")
                rows = cur.fetchall()
            except sqlite3.OperationalError as oe:
                logging.error(f"No settings table: {oe}")
                rows = []
            conn.close()
            ...
            for row in rows:
                uid = row['user_id']
                try:
                    await bot.send_message(uid, msg, reply_markup=keyboard, parse_mode="Markdown")
                except Exception as ex:
                    logging.error(f"Failed to send update to {uid}: {ex}")
